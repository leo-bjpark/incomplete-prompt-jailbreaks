...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Neuron steering with positive-minus-negative neuron set.")
    ...
    parser.add_argument("--neuron_path_neg", type=str, required=True)
...
def main() -> None:
    args = parse_args()
    save_dir = (
        f"outputs/run6_neuron_steering_pos_and_neg/{args.neuron_path}/{args.neuron_path_neg}/"
    ...
    correlation_scores_neg = load_correlation_scores(args.run4_base_dir, args.neuron_path_neg, args.model_name)
    indices_pos = get_threshold_based_neuron_indices(correlation_scores, args.at, args.threshold)
    indices_neg = get_threshold_based_neuron_indices(correlation_scores_neg, args.at, args.threshold)
